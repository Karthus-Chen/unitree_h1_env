...
class PlacoTeleopController(BaseTeleopController):
    ...
    # def run_single_step_w01(self):
    #     start_time = time.time()
    #     self._update_ik()
    #     torso = max(-0.1, min(self.head_delta, 0.4))
    #     waist = 1.2*max(0, min(self.head_delta, 0.4))
    #     self.placo_robot.state.q[7]=self.q_init[0] + torso
    #     self.placo_robot.state.q[8]=self.q_init[1] + waist
    #     self._send_command()
    #     end_time = time.time()
    #     time.sleep(max(0, self.dt - (end_time - start_time))) 
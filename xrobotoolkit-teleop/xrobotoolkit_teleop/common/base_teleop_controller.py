...
class BaseTeleopController(abc.ABC):
    ...
    def get_active_state(self):
        A_button = self.xr_client.get_button_state_by_name("A")
        left_arm_config = self.manipulator_config.get('left_arm', {})
        control_trigger = left_arm_config.get('control_trigger')
        xr_grip_val = self.xr_client.get_key_value_by_name(control_trigger)
        active_left = xr_grip_val > 0.9
        right_arm_config = self.manipulator_config.get('right_arm', {})
        control_trigger = right_arm_config.get('control_trigger')
        xr_grip_val = self.xr_client.get_key_value_by_name(control_trigger)
        active_right = xr_grip_val > 0.9

        return A_button,active_left,active_right
    ...
    def _update_ik(self):
        ...
        ### new add ###
        A_button, self.active['left_arm'], self.active['right_arm'] = self.get_active_state()
        if(A_button):
            self.reset_flag=True
        if(self.active['left_arm'] or self.active['right_arm']):
        ...
        if(self.reset_flag):
            ...
            for src_name, config in self.manipulator_config.items():
                ...
                if self.active[src_name]:
                    ...
                    ###### 计算head_delta 给 torso ######
                    head_pose = self.xr_client.get_pose_by_name("headset")
                    head_z=head_pose[1]
                    qurt=np.array([head_pose[3],head_pose[4],head_pose[5],head_pose[6]])
                    if(self.first_active_flag==False):
                        self.first_active_flag=True
                        self.init_z=head_z   
                    self.head_delta=self.init_z-head_z
                    # print(self.head_delta)
    ...
    def reset_robot(self):
        ...
        # print("Robot has been reset to the initial configuration.")
